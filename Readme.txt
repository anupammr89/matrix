...
Establish a MATRIX server
---------------------------------------------
Note: the $protocol could only be "TCP" or "UDP"

Server:
Simply execute ./server_general $port $memberListFile $configFile $protocol $username $logging $max_jobs_per_package to start a instance of ZHT server node.
